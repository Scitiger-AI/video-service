...
class AliyunProvider(ModelProvider):
    ...
    async def validate_parameters(self, model: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 检查是什么类型的模型
        is_text_to_video = model.startswith("wanx2.1-t2v")
        is_image_to_video = model.startswith("wanx2.1-i2v")
        is_keyframe_to_video = model.startswith("wanx2.1-kf2v")
        
        # 检查必要参数
        if "prompt" not in validated:
            # 首尾帧生成视频可以不需要提示词
            if not is_keyframe_to_video:
        ...
        if "duration" not in validated:
            ...
            # 确保持续时间在合理范围内
            if is_text_to_video or is_image_to_video:
                # T2V和I2V模型支持3-5秒
    ...
    async def call_model(self, model: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 添加首尾帧模型特殊参数
            if model.startswith("wanx2.1-kf2v") and "obj_or_bg" in validated_params:
